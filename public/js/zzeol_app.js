...
let USE_MOCK = false;
let USE_REST = true;                 // 웹소켓 대신 항상 REST 폴링
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  input.addEventListener('keydown', (e) => {
    if (e.key === 'ArrowDown') { moveAuto(1); e.preventDefault(); }
      ...
      if (auto.classList.contains('show')) { pickAuto(); e.preventDefault(); }
      else { const first = el.listBody.querySelector('tr'); if (first) first.click(); }
...
async function getATR(code){
  try{
    ...
    const r=await fetch(`https://api.upbit.com/v1/candles/minutes/${ATR_TF}?market=${code}&count=${cnt}`);
    const arr=await r.json(); if(!Array.isArray(arr)||arr.length<cnt) return null;
    const trs=[]; for(let i=0;i<arr.length-1;i++){ const c=arr[i], p=arr[i+1];
      trs.push(Math.max(c.high_price-c.low_price, Math.abs(c.high_price-p.trade_price), Math.abs(c.low_price-p.trade_price))); }
...
async function updateLevelsWithATR(price, code){
  const atr = await getATR(code); const p=Number(price);
  if(!atr || !isFinite(p)) return;
...
function renderOrderbook(ob){
  ...
  const a = document.querySelector('#ask-table tbody'), b = document.querySelector('#bid-table tbody');
  if(!a||!b) return;
...
async function loadMarkets(retry=0){
  try{
    ...
    searchIndex = markets.map(m=>{
      const sym=m.market.replace(baseMarket+'-',''), ko=m.korean_name||'', en=m.english_name||'';
...
function showAuto(s){
  const q=norm(s);
  const items=autoItems.filter(x=>norm(x.ko).includes(q)||norm(x.en).includes(q)||norm(x.sym).includes(q)).slice(0,20);
...
async function restFetchTicker(code){
  try{
    ...
    const js=await r.json(); const t=js && js[0]; if(!t) return;
...
async function restFetchOrderbook(code){
  try{
    ...
    const js=await r.json(); const ob=js && js[0]; if(!ob) return;
    renderOrderbook(ob);
  }catch(_){}
}
function startRestLoop(code){
  clearRestLoop(); if(!code) return;
...
async function refreshListPrices(){
  ...
  runPumpScanner(tmap); // 급등감지 함께 실행
}
...
function percentChange(code, mins){
  const arr=priceHist[code]; if(!arr||arr.length<2) return 0;
  const now=Date.now(), target=now-mins*60*1000;
...
function inflowRate(code, mins=3){
  const arr=priceHist[code]; if(!arr||arr.length<2) return 0;
  const now=Date.now(), target=now-mins*60*1000;
  let base=arr[0]; for(let i=0;i<arr.length;i++){ if(arr[i].t>=target){ base=arr[i]; break; } }
...
function pumpScore(pc1,pc3,pc5,flow){
  let s=0; s+=Math.max(0,pc1)*2.2; s+=Math.max(0,pc3)*1.4; s+=Math.max(0,pc5)*1.0;
  s+=Math.min(5,(flow/1_000_000_000)*1.8); if(pc1>7&&pc3<9) s-=2.0; return s;
}
