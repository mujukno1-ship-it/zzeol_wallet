...
<title>사토시의지갑 v10.2 — 검색 우선 / 고정코인 아래 / 예열 상단반영 / 사람형 상시</title>
<style>
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="row">
      ...
      <button id="btnHot">급등</button>
      <button id="btnFall">급락</button>
      <button id="btnReset">초기화</button>
      <span class="badge">빠른검색: <a href="#" id="qSHIB" style="color:#cfe2ff">SHIB</a> · <a href="#" id="qADA" style="color:#cfe2ff">ADA</a></span>
    </div>
  ...
  <div class="card">
    ...
    <table>
      ...
      <tbody id="pinnedBody">
        <tr><td colspan="9" class="status">고정된 코인이 없습니다. 위 표에서 ★을 눌러 고정해 보세요.</td></tr>
      </tbody>
    </table>
  </div>

  <!-- 🔥 예열 자동예측 — 캐시 복원 + 실시간 / 신규는 상단 -->
  <div class="card">
    ...
    <table>
      ...
      <tbody id="preheatBody">
        <tr><td colspan="9" class="status">예열 후보 없음(감시 중)</td></tr>
      </tbody>
    </table>
  </div>

  <!-- 🧭 사람형 AI 표 — 접속 즉시 채움 + 새후보 상단 반영 -->
  <div class="card">
...
const DEFAULT_SEED=['KRW-BTC','KRW-ETH','KRW-ADA','KRW-XRP','KRW-SOL','KRW-SHIB']; // 접속 즉시 사람형 표 보장

/* ===== 상태/유틸 ===== */
const el={
  q:byId('q'), status:byId('status'), net:byId('net'),
  tbody:byId('tbody'), pinnedBody:byId('pinnedBody'),
...
const scanPreheat = debounce(function(){
  ...
  renderPreheat(list); renderPinned(); renderAI(); // 자동 상단 반영
}, 250);
...
function wirePins(tbody){
  tbody.querySelectorAll('.pin').forEach(el=>{
    el.onclick=()=>{
      ...
      savePinned(); renderPinned(); renderAI(); // 즉시 반영
    };
  });
}
...
function renderPinned(){
  ...
  if(!codes.length){ body.innerHTML='<tr><td colspan="9" class="status">고정된 코인이 없습니다. 위 표에서 ★을 눌러 고정해 보세요.</td></tr>'; return; }
  body.innerHTML='';
...
async function analyzeHuman(market){
  const [h1]=await Promise.all([fetchUpbit(API.CANDLE(market,'1h'))]);
...
function onSearch(){
  ...
  renderTable(list);
  renderAI(list.slice(0,12).map(m=>m.market)); // 검색 결과를 사람형 표에도 즉시 반영
}
...
(async function start(){
  ...
  renderAI();              // 접속 즉시 사람형 표가 비지 않도록 보장
  scanPreheat();           // 이후 신규 예열은 상단 반영
